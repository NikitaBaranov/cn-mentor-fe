        ...
        <div key={s.id} className={"card border-success m-2"}>
            {edit ? (<>
                        ...
                        <div className="col">
                            <div className="input-group">
                                <input
                                    ...
                                    onChange={e => setS({
                                        ...s, gutHubUserName: e.target.value
                                    })}
                                />
                            </div>
                        </div>
                ...
                <div className="card-header text-success aParent" id={s.id} name={s.id}>
                    ...
                    {s.gutHubUserName ? (<a href={`https://github.com/${s.gutHubUserName}`}
                                            target="_blank"
                                            className={"card-link"} rel="noreferrer">
                        <FaGithub/>
                    </a>) : <a href={`#`}
                ...
                <div className="card-body">
                    <div className="container">
                        {student.gitHubUser && (<div className="row">
                            <GigHubActivityComponent
                                gitHubUserId={student.gitHubUser.id}
                                daysToShow={60}
                            />
                        </div>)}
                        {student.gitHubRepos ? <>
                            {student.gitHubRepos
                                ...
                                .map(repo => (<GitHubRepoRowComponent
                                    key={repo.id}
                                    gitHubUserId={student.gitHubUser.id}
                                    gitHubRepo={repo}/>))}
                        </> : <></>}
